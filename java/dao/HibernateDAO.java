  ...
  implements IHibernateDAO
{
  ...
  public void executeUpdateBySQL(String sql,String[] paramNames ,Object[] valores) throws DAOException
  {
    try
    {
  ...
  public List findAll(Class clazz, List orders) throws DAOException {
    ...
    try {
      /*Criteria criteria = session.createCriteria("from " + clazz.getName();
      for (Iterator iter = orders.iterator(); iter.hasNext();) {
        criteria.addOrder((Order)iter.next());
      }*/
      String crit = "from " + clazz.getName();
      if (orders.size() > 0)
      {
    	  crit = crit + " order by ";
      }
      for (Iterator iter = orders.iterator(); iter.hasNext();) {
          ...
          crit = crit + order.getPropertyName() + " " + (order.isAscending() ? "asc" : "desc");
          crit = crit + ",";
        }
      
      crit = crit.substring(0, crit.length() - 1);
      objs = getCurrentSession().createQuery(crit).list();
    } catch (HibernateException ex) {
  ...
  public List findByCriterionsDistinct(Class clazz, List restrictions) throws DAOException
  {
    List objs = new ArrayList();
    try
    {
  ...
  public void saveOrUpdate(Object persistentObject) throws DAOException {
    try {
        ...
        else if(getCurrentSession().getTransaction().isActive()==false)
            getCurrentSession().beginTransaction();
